...
class Producto:
    def _init_(self, archivo, indice):
    ...
    def construir_indice_proveedores(self):
        """Construye un diccionario donde cada proveedor es una clave y sus productos son valores"""
        indice_proveedores = {}
        ...
        with open(self.archivo.file_name, 'r') as archivo:
            for linea in archivo:
                ...
                if len(partes) > 6:
                    proveedor = partes[6].strip().lower()  # Convertimos a minúsculas para evitar errores
                    if proveedor not in indice_proveedores:
                        indice_proveedores[proveedor] = []  # Creamos la clave si no existe
                    indice_proveedores[proveedor].append(linea.strip())  # Agregamos el producto a la lista

        return indice_proveedores  # Retornamos el diccionario construido
    
    def buscar_por_nombre(self, nombre):
        productos_encontrados = []
        with open(self.archivo.file_name, 'r') as archivo:  # Eliminamos 'encoding'
            for linea in archivo:
                partes = linea.strip().split(',')
                if len(partes) > 1 and partes[1].lower() == nombre.lower():  # Comparación sin mayúsculas/minúsculas
    ...
    def buscar_por_categoria(self, categoria):
        productos_encontrados = []
        with open(self.archivo.file_name, 'r') as archivo:  # Sin encoding
            for linea in archivo:
                partes = linea.strip().split(',')
                if len(partes) > 2 and partes[2].lower() == categoria.lower():  # Comparación sin distinción de mayúsculas/minúsculas
                    productos_encontrados.append(linea.strip())
...
def register_provider(archivo_proveedores):
    print("\n--- Registrar Proveedor ---")
    id_proveedor = str(Archivo(archivo_proveedores).contar_lineas())
